...
Ddf = pd.read_csv('Ddf_v002.csv', index_col=0)
...
passage_pattern = r"^((?!(D\.|[A-Z]{2,}))|\"(\s|[A-Za-z]))\w+"
...
# s = len(index_list_s) # 432 - All section_ref lines captured
# h = len(index_list_h)# 
# p = len(index_list_p)
# x = len(index_list_x)
...
Ddf_v002x.to_csv('Ddf_v002x.csv')