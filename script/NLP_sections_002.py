...
df.doc[308] = ''
# type(df.doc[308]) # Thematic section with id 308 had Greek characters only which were removed
...
# print(df.length_unique.mean())        # Average length: 355.71 lemmas
# print(df.length_unique.median())      # Median length: 280.50 lemmas
# print(df.head())
...
# ps.head(30)                           # Length reaches the 100-word mark between 20% and 21%

# Create list of IDs of thematic sections which are shorter than 100 unique lemmas
drop_short = df.index[df.length_unique <= 100]
# len(drop_short)                       # 92 thematic sections below the 100-word mark
...
for i in sf.index:
    ...
    top_terms.extend(t)
top_terms = list(set(top_terms))    # 3868 unique terms

# Keep top terms in the tfidf matrix
sf = sf[top_terms]                  # 3868 columns (unique terms) and 340 rows (thematic sections)
